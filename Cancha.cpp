...
Point CCancha::GetPosicionAleatoriaLibre() {
    srand(time(NULL)); // Inicializamos la semilla aleatoria una vez

    while (true) {
        int fila = rand() % FILAS;
        int columna = rand() % COLUMNAS;
                                                                                            /*PONER EN POSICION ALEATORIA*/
        EElementos tipo = cancha[fila][columna]->getTipo();

        if (tipo == EElementos::piso || tipo == EElementos::libre) {
            // Convertimos a coordenadas en píxeles
            int x = columna * ANCHOIMAGEN;
            int y = fila * ALTOIMAGEN;
            return Point(x, y);
        }
    }
}